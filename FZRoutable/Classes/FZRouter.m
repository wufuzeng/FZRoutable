...
+(NSString*)localProtocol{
    if ([FZRouter globalRouter].localScheme.length == 0) {
        @throw [NSException exceptionWithName:@"FZRouter Error:"
                                       reason:@"localScheme is nil"
                                     userInfo:nil];
    }else{
        return [NSString stringWithFormat:@"%@://",[FZRouter globalRouter].localScheme];
    }
}
...
+(NSString*)localPath:(NSString *)path{
    if (path.length == 0) {
        ...
        return [[FZRouter localProtocol] stringByAppendingString:path];
    }
}
...
#pragma clang diagnostic pop
    ...
    } else if ([entry.routerUrl hasPrefix:[FZRouter localProtocol]]) {
